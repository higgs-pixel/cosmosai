...
export default function StarGazeView({ observer: initialObserver }: StarGazeViewProps) {
  ...
  return (
    <div className="relative w-full h-full overflow-hidden select-none font-sans bg-[#020308] text-white">
      {/* ─────────────────────────────────────────────────────────────────── */}
      ...
      <div className="relative w-full h-full min-h-[500px]">
        ...
        {/* ── TOP SPATIAL LIQUID GLASS MISSION CONTROL NAV ── */}
        <TopFloatingNav
          observer={currentObserver}
          currentDate={currentDate}
        ...
        {/* ── LEFT FLOATING MISSION CONTROL PANEL (COLLAPSIBLE) ── */}
        <LeftControlPanel
          observer={currentObserver}
          onSelectObserver={(obs) => {
            setCurrentObserver(obs);
            showToast(`Site: ${obs.name}`);
          }}
          onRequestGps={handleRequestGps}
          presetLocations={PRESET_LOCATIONS}
          sunCoords={observerSunCoords}
          nakedEyeCount={nakedEyeCount}
          currentBearing={mobileOrientation ? mobileOrientation.heading : headingAzimuth}
          currentPitch={mobileOrientation ? mobileOrientation.pitch : 25}
          is180DomeView={is180DomeView}
          onToggle180DomeView={toggle180DomeView}
          showLabels={showLabels}
          onToggleLabels={() => setShowLabels((prev) => !prev)}
        ...
        {/* ── FLOATING COMPASS HUD ROSE (AEROSPACE RETICLE) ── */}
        <FloatingCompassHUD
          ...
          selectedSat={detailedSelectedSat}
          className="absolute bottom-16 left-4 sm:left-6 z-20"
        />

        {/* ── BOTTOM SPACECRAFT STATUS HUD ── */}
        <BottomSpacecraftHUD
          nakedEyeCount={nakedEyeCount}
        ...
        {/* 2D PLANISPHERE RADAR OVERLAY */}
        {showRadar && (
          <div className="absolute bottom-16 left-40 sm:left-44 z-20 pointer-events-auto flex flex-col items-center gap-1.5 shadow-2xl">
            <Planisphere2DRadar
              satellites={allSatellites}
              selectedSatId={selectedSat ? selectedSat.id : null}
              onSelectSat={(sat) => handleTrackSatellite(sat)}
        ...
        {showSimDock && (
          <div className="absolute bottom-16 left-1/2 -translate-x-1/2 z-40 w-[340px] sm:w-[410px] flex flex-col gap-2.5 p-3.5 rounded-2xl bg-slate-950/75 border border-white/[0.14] backdrop-blur-3xl shadow-[0_16px_48px_0_rgba(0,0,0,0.6),0_0_30px_rgba(16,185,129,0.2),inset_0_1px_0_0_rgba(255,255,255,0.15)] pointer-events-auto animate-in fade-in slide-in-from-bottom-4 duration-200">
            {/* Header label */}
        ...
        {/* ── RIGHT TELEMETRY DOCK (LIQUID GLASS CAPSULE CARDS & FILTER TABS) ── */}
        <RightTelemetryDock
          ...
          selectedCategory={selectedCategory as "Visible in 24 Hours" | "Naked-Eye Visible" | "Overhead Now"}
          onSelectCategory={(cat) => setSelectedCategory(cat)}
          visible24hCount={visible24hCount}
          nakedEyeCount={nakedEyeCount}
          visibleCount={visibleCount}
          lastRefreshedDate={lastRefreshedDate}
          onRefreshTles={() => loadRealTimeTles(true)}
