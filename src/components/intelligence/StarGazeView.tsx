...
// ─────────────────────────────────────────────────────────────────────────────
// STARGAZER SYSTEM MANUAL & ASTRODYNAMICS GLOSSARY (STARGAZE THEME)
// ─────────────────────────────────────────────────────────────────────────────
interface GlossaryItem {
  id: string;
  name: string;
  symbol: string;
  category: "coordinates" | "orbits" | "optics" | "tracking";
  definition: string;
  note: string;
  badgeColor: string;
}
...
const GLOSSARY_TERMS: GlossaryItem[] = [
  // ── Coordinates ──
  {
    ...
    badgeColor: "cyan",
    definition: "Equatorial astrometric coordinate measuring eastward angular distance along the celestial equator from the Vernal Equinox (First Point of Aries). Projected terrestrial longitude onto the celestial sphere.",
    note: "Expressed in sidereal hours, minutes, and seconds. Fixed stars retain nearly constant RA across centuries (excluding precession).",
  },
  {
    id: "dec",
    name: "Declination",
    symbol: "Dec, δ (-90° to +90°)",
    category: "coordinates",
    badgeColor: "cyan",
    definition: "Equatorial coordinate measuring the angular distance north (+) or south (-) from the celestial equator perpendicular to the celestial equator plane.",
    note: "Celestial equator is 0°, North Celestial Pole (near Polaris) is +90°, and South Celestial Pole is -90°.",
  },
  {
    id: "el",
    name: "Elevation Angle",
    symbol: "El / Alt (0° to 90°)",
    category: "coordinates",
    badgeColor: "cyan",
    definition: "Topocentric horizontal coordinate measuring the vertical angular height of a spacecraft above the observer's mathematical horizon. 0° = Horizon, 90° = Zenith.",
    note: "Optical observing requires El ≥ 10° to avoid dense atmospheric haze and severe terrestrial light pollution.",
  },
  {
    id: "az",
    ...
    category: "coordinates",
    badgeColor: "cyan",
    definition: "Topocentric horizontal compass angle measured clockwise from True Geographic North (0° N, 90° E, 180° S, 270° W) along the local tangent horizon plane.",
    note: "Directly drives telescope mount azimuth rotators and ground station parabolic dish antennas.",
  },
  {
    id: "zenith",
    name: "Zenith & Nadir",
    ...
    category: "coordinates",
    badgeColor: "cyan",
    definition: "Zenith is the imaginary point on the celestial sphere directly above the observer perpendicular to the horizon. Nadir is the opposite point directly beneath the observer.",
    note: "Near-zenith passes have the shortest slant range, minimum atmospheric extinction, and highest angular slew speed.",
  },
  {
    id: "meridian",
    name: "Local Celestial Meridian",
    symbol: "Meridian (Az 0° / 180°)",
    category: "coordinates",
    badgeColor: "cyan",
    definition: "Great circle on the celestial sphere passing through the North Celestial Pole, the observer's Zenith, and the South Celestial Pole, dividing the sky into Eastern and Western hemispheres.",
    note: "When an orbiting body crosses the meridian, it culminates (attains maximum elevation for that pass).",
  },
  ...
  // ── Orbital Mechanics ──
  {
    ...
    category: "orbits",
    badgeColor: "purple",
    definition: "Standardized format of two 69-character lines of ASCII text containing Keplerian mean orbital elements at an epoch timestamp, generated by US Space Command and NORAD.",
    note: "Subject to rapid decay due to thermospheric drag. Fresh TLEs are updated every 24-72 hours via CelesTrak.",
  },
  {
    id: "sgp4",
    name: "SGP4 Propagation Kernel",
    symbol: "SGP4 / SDP4",
    category: "orbits",
    badgeColor: "purple",
    definition: "Simplified General Perturbations-4 analytical orbit propagation model calculating satellite position and velocity vectors while factoring in Earth oblateness (J2, J3, J4 zonal harmonics) and atmospheric drag.",
    note: "Translates epoch TLE data into True Equator Mean Equinox (TEME) coordinates for observer topocentric projection.",
  },
  {
    id: "inc",
    ...
    category: "orbits",
    badgeColor: "purple",
    definition: "Angle between a satellite's orbital plane and Earth's equatorial plane. 0° = Equatorial orbit, 90° = Polar orbit, >90° = Retrograde orbit.",
    note: "The International Space Station orbits at i = 51.6°, allowing ground coverage over 90% of Earth's populated latitudes.",
  },
  {
    id: "perigee_apogee",
    name: "Perigee & Apogee",
    ...
    category: "orbits",
    badgeColor: "purple",
    definition: "The points in an elliptical orbit closest to (Perigee) and farthest from (Apogee) the Earth's center of mass.",
    note: "LEO perigee ranges from 300 to 500 km, while GEO orbits maintain near-circular perigee and apogee at 35,786 km.",
  },
  {
    id: "ecc",
    name: "Orbital Eccentricity",
    symbol: "e (0.000 - 1.000)",
    category: "orbits",
    badgeColor: "purple",
    definition: "Dimensionless parameter defining the geometric elongation of an orbital ellipse. e = 0 is a circle, 0 < e < 1 is an ellipse, e >= 1 is parabolic/hyperbolic.",
    note: "Most Earth observation and communication constellations maintain near-circular trajectories with e < 0.002.",
  },
  {
    id: "raan",
    name: "RAAN",
    ...
    category: "orbits",
    badgeColor: "purple",
    definition: "Right Ascension of Ascending Node: Angle measured eastward at the equator from the Vernal Equinox to the point where the orbit crosses from South to North.",
    note: "Precesses over time due to Earth's equatorial bulge (J2 perturbation); calibrated in Sun-synchronous orbits.",
  },
  {
    id: "semi_major",
    ...
    category: "orbits",
    badgeColor: "purple",
    definition: "One half of the major axis of an orbital ellipse. Defines orbital size and directly determines the orbital period according to Kepler's Third Law.",
    note: "For LEO satellites (a ≈ 6,700-7,000 km), orbital period is approximately 90 to 95 minutes (~15-16 orbits/day).",
  },
  ...
  // ── Optics & Magnitude ──
  {
    ...
    category: "optics",
    badgeColor: "amber",
    definition: "Logarithmic scale of astronomical brightness where a step of 5 magnitudes equals an exact 100x brightness ratio. Lower or negative values represent brighter objects.",
    note: "Naked-eye threshold in pristine dark skies is +6.0. The ISS peaks around -3.5, visible even in light-polluted urban skies.",
  },
  {
    id: "sunlit_eclipse",
    name: "Sunlit vs. Eclipse Phase",
    symbol: "Umbra / Penumbra",
    category: "optics",
    badgeColor: "amber",
    definition: "Binary orbital condition indicating whether the spacecraft is illuminated by direct sunlight or immersed in Earth's shadow cone (umbra).",
    note: "A satellite at 70° elevation is completely invisible if eclipsed, as it emits no intrinsic light.",
  },
  {
    id: "flare",
    name: "Specular Flare & Glints",
    symbol: "Flare (Δmag < -4)",
    category: "optics",
    badgeColor: "amber",
    definition: "Transient flash of reflected sunlight caused by planar solar arrays or thermal radiators momentarily acting as a mirror toward the observer.",
    note: "Can spike a dim magnitude +5 satellite into a brilliant magnitude -2 to -4 burst lasting 5 to 20 seconds.",
  },
  {
    id: "phase_angle",
    name: "Solar Phase Angle",
    symbol: "β (0° to 180°)",
    category: "optics",
    badgeColor: "amber",
    definition: "The angle subtended between the Sun, the target satellite, and the ground observer (Sun-Target-Observer angle).",
    note: "Small phase angles produce near-full illumination (forward scattering), substantially brightening visible passes.",
  },
  {
    id: "extinction",
    ...
    category: "optics",
    badgeColor: "amber",
    definition: "The absorption and scattering of light by Earth's atmospheric column, proportional to the secant of the observer's zenith angle.",
    note: "Diminishes target brightness by 1.5 to 2.5 magnitudes near the horizon (El < 10°) compared to zenith transit.",
  },

  // ── Tracking & Passes ──
  {
    id: "aos_los",
    name: "AOS & LOS (Signal Acquisition)",
    symbol: "AOS / LOS (UTC)",
    category: "tracking",
    badgeColor: "emerald",
    definition: "AOS (Acquisition of Signal) is the moment a satellite rises above the local elevation mask. LOS (Loss of Signal) is the moment it sinks below the horizon.",
    note: "Defines the active observation or radio communication window, typically 6 to 14 minutes for LEO orbits.",
  },
  {
    id: "tca",
    ...
    category: "tracking",
    badgeColor: "emerald",
    definition: "The exact instant during a pass when slant range distance is at its minimum, coinciding with the pass peak elevation angle.",
    note: "Represents peak optical brightness, maximum Doppler zero-crossing in RF, and fastest angular slew velocity.",
  },
  {
    id: "slant_range",
    name: "Slant Range",
    ...
    category: "tracking",
    badgeColor: "emerald",
    definition: "The direct Euclidean distance from the observer's topocentric ground antenna or eye directly to the orbiting spacecraft.",
    note: "Slant range can exceed 2,200 km at AOS/LOS horizon, shrinking to 400-500 km at overhead zenith culmination.",
  },
  {
    id: "doppler",
    name: "Doppler Frequency Shift",
    symbol: "Δf (kHz)",
    category: "tracking",
    badgeColor: "emerald",
    definition: "The frequency offset observed in satellite radio telemetry caused by high relative line-of-sight velocity toward or away from the station.",
    note: "Transceiver frequencies must be tuned higher (+Δf) as satellite approaches, crossing zero at TCA, and lower (-Δf) during departure.",
  },
  {
    id: "criteria",
    name: "Naked-Eye Visibility Triad",
    symbol: "Dark + Lit + Mag",
    category: "tracking",
    badgeColor: "emerald",
    definition: "The three physical criteria required for naked-eye sighting: (1) Sun ≤ -6° below horizon (dusk/night), (2) Satellite in direct sunlight, (3) V_mag ≤ +6.0.",
    note: "The Stargaze observation engine strictly calculates and displays this triad for all visual satellite alerts.",
  },
  {
    id: "ssp",
    name: "Ground Track & Sub-Satellite Point",
    symbol: "SSP (lat, lon)",
    category: "tracking",
    badgeColor: "emerald",
    definition: "The instantaneous geographic point on Earth's surface directly beneath the satellite (nadir point). Traced over time, this forms the ground track.",
    note: "Due to Earth's eastward rotation beneath the orbital plane, consecutive ground tracks shift westward by ~23° to 25° per orbit.",
  },
];
...
function StarGazerManualModal({
  ...
  const filteredGlossary = useMemo(() => {
    return GLOSSARY_TERMS.filter((term) => {
      ...
      const matchesQuery =
        ...
        term.symbol.toLowerCase().includes(q) ||
  ...
  const categoryCounts = useMemo(() => {
    return {
      ...
      coordinates: GLOSSARY_TERMS.filter((t) => t.category === "coordinates").length,
      orbits: GLOSSARY_TERMS.filter((t) => t.category === "orbits").length,
      optics: GLOSSARY_TERMS.filter((t) => t.category === "optics").length,
      tracking: GLOSSARY_TERMS.filter((t) => t.category === "tracking").length,
    };
  }, []);

  return (
    <AnimatePresence>
      <div className="fixed inset-0 z-[10000] flex items-center justify-center p-3 sm:p-6 bg-black/85 backdrop-blur-xl pointer-events-auto font-sans select-none">
        <motion.div
          initial={{ opacity: 0, scale: 0.96, y: 12 }}
          animate={{ opacity: 1, scale: 1, y: 0 }}
          exit={{ opacity: 0, scale: 0.96, y: 12 }}
          transition={{ duration: 0.22, ease: [0.16, 1, 0.3, 1] }}
          className="relative w-full max-w-5xl max-h-[90vh] bg-[#02040a] border border-zinc-800 shadow-[0_25px_70px_rgba(0,0,0,0.95),0_0_50px_rgba(0,229,255,0.06)] flex flex-col overflow-hidden text-zinc-100"
        >
          {/* Top Hairline Spectral Accent Bar */}
          <div className="absolute top-0 inset-x-0 h-[2px] bg-gradient-to-r from-cyan-500 via-emerald-400 to-amber-500 opacity-90" />

          {/* Modal Header */}
          <div className="flex items-center justify-between px-5 sm:px-6 py-4 border-b border-zinc-800 bg-black/80 shrink-0">
            <div className="flex items-center gap-3">
              <div className="p-2 bg-cyan-950/40 border border-cyan-500/40 text-cyan-400 shadow-[0_0_15px_rgba(0,229,255,0.2)]">
                <Globe className="h-5 w-5" />
              </div>
              <div>
                <div className="flex items-center gap-2 flex-wrap">
                  <h2 className="text-xs sm:text-sm font-black tracking-widest text-white uppercase font-mono">
                    STARGAZER ASTRODYNAMICS & SYSTEM LEXICON
                  </h2>
                  <span className="px-2 py-0.5 text-[9px] font-mono font-bold bg-cyan-950/50 text-cyan-300 border border-cyan-500/40 tracking-wider">
                    v2.4 TELEMETRY ENGINE
                  </span>
                  <span className="hidden md:inline px-2 py-0.5 text-[9px] font-mono font-bold bg-emerald-950/40 text-emerald-400 border border-emerald-500/30 tracking-wider">
                    SGP4 KERNEL VERIFIED
                  </span>
                </div>
                <p className="text-[10px] sm:text-[11px] text-zinc-400 font-mono tracking-wide mt-0.5">
                  Topocentric Equatorial System • Orbit Mechanics • Photometric Optics & Ground Track Ephemeris
                </p>
              </div>
            </div>
            <button
              onClick={onClose}
              className="w-8 h-8 bg-zinc-900 border border-zinc-800 hover:border-zinc-700 hover:bg-zinc-800 text-zinc-400 hover:text-white text-xs flex items-center justify-center transition cursor-pointer"
              title="Close Manual"
            >
              <X className="h-4 w-4" />
            </button>
          ...
          {/* Tab Navigation Bar */}
          <div className="flex items-center gap-1 sm:gap-2 px-5 sm:px-6 py-2 bg-black/95 border-b border-zinc-800 shrink-0 font-mono text-xs overflow-x-auto scrollbar-none">
            {tabs.map((tab) => {
              const Icon = tab.icon;
              ...
              return (
                <button
                  ...
                  className={`relative flex items-center gap-2 px-3 sm:px-4 py-1.5 transition-all duration-150 cursor-pointer text-xs whitespace-nowrap ${
                    isActive
                      ? "border border-cyan-500/50 bg-cyan-950/30 text-cyan-300 font-bold shadow-[0_0_15px_rgba(0,229,255,0.15)]"
                      : "border border-transparent text-zinc-400 hover:text-zinc-200 hover:border-zinc-800 hover:bg-zinc-900/40"
                  }`}
                >
          ...
          <div className="flex-1 overflow-y-auto p-5 sm:p-6 text-xs leading-relaxed font-sans bg-[#02040a]">
              ...
              >
                ...
                {activeTab === "terms" && (
                  <div className="space-y-4">
                    ...
                    <div className="p-3.5 bg-zinc-950/80 border border-zinc-800 space-y-3">
                      <div className="flex flex-col sm:flex-row items-stretch sm:items-center justify-between gap-3">
                        ...
                        <div className="relative flex-1">
                          ...
                          <input
                            ...
                            placeholder="Search 24 concepts, symbols (e.g. RA, SGP4, V_mag, TCA)..."
                            className="w-full pl-9 pr-8 py-1.5 bg-black border border-zinc-800 text-xs font-mono text-white placeholder:text-zinc-600 focus:outline-none focus:border-cyan-500/60 transition"
                          />
                          {glossarySearch && (
                            <button
                              onClick={() => setGlossarySearch("")}
                              className="absolute right-2.5 top-1/2 -translate-y-1/2 text-zinc-500 hover:text-white transition cursor-pointer"
                            >
                              <X className="h-3 w-3" />
                            </button>
                          )}
                        ...
                        {/* Total Match Badge */}
                        <div className="flex items-center gap-2 text-[10px] font-mono text-zinc-400 shrink-0 self-end sm:self-center">
                          <span className="text-zinc-500 uppercase">Showing:</span>
                          <span className="px-2 py-0.5 bg-black border border-zinc-800 text-cyan-300 font-bold">
                            {filteredGlossary.length} of {GLOSSARY_TERMS.length} TERMS
                          </span>
                        </div>
                      </div>
                      ...
                      <div className="flex items-center gap-1.5 flex-wrap font-mono text-[10px]">
                        {[
                          ...
                          { id: "coordinates", label: `COORDINATES (${categoryCounts.coordinates})`, color: "cyan" },
                          { id: "orbits", label: `ORBITAL MECHANICS (${categoryCounts.orbits})`, color: "purple" },
                          { id: "optics", label: `OPTICS & MAGNITUDE (${categoryCounts.optics})`, color: "amber" },
                          { id: "tracking", label: `PASS TRACKING (${categoryCounts.tracking})`, color: "emerald" },
                        ].map((cat) => {
                          const isSelected = glossaryCategory === cat.id;
                          return (
                            <button
                              key={cat.id}
                              onClick={() => setGlossaryCategory(cat.id as any)}
                              className={`px-2.5 py-1 uppercase tracking-wider transition-all cursor-pointer border ${
                                isSelected
                                  ? "bg-cyan-950/40 border-cyan-500/60 text-cyan-300 font-bold shadow-[0_0_10px_rgba(0,229,255,0.2)]"
                    ...
                    {filteredGlossary.length > 0 ? (
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                        {filteredGlossary.map((item) => {
                          ...
                          const categoryLabel =
                            item.category === "coordinates"
                              ? "Astrometric Coordinates"
                              : item.category === "orbits"
                          ...
                          return (
                            <div
                              key={item.id}
                              className="p-4 bg-zinc-950/80 border border-zinc-800/90 hover:border-cyan-500/40 transition duration-200 space-y-2.5 flex flex-col justify-between group"
                            >
                              <div>
                                <div className="flex items-start justify-between gap-2">
                                  ...
                                  <span className={`font-mono text-[9px] font-bold px-2 py-0.5 border shrink-0 ${badgeBorder}`}>
                                    {item.symbol}
                                  </span>
                                </div>
                                <p className="text-zinc-300 text-[11px] leading-relaxed font-sans mt-2">
                                  {item.definition}
                                </p>
                              </div>

                              <div className="mt-2.5 p-2 bg-black/80 border-l-2 border-cyan-400/80 border-t border-r border-b border-zinc-900 font-mono text-[10px] text-zinc-400 leading-relaxed">
                                <span className="text-cyan-400 font-bold uppercase tracking-wider">FIELD INSIGHT: </span>
                                <span>{item.note}</span>
                              </div>
                            </div>
                          );
                      ...
                      <div className="p-8 text-center bg-zinc-950/60 border border-zinc-800 space-y-2">
                        <Globe className="h-6 w-6 text-zinc-600 mx-auto animate-pulse" />
                        <div className="font-mono text-xs text-zinc-400">No astrodynamic terms found matching &quot;{glossarySearch}&quot;</div>
                        <button
                          onClick={() => {
                            setGlossarySearch("");
                            setGlossaryCategory("all");
                          }}
                          className="mt-2 px-3 py-1 bg-black border border-cyan-500/40 text-cyan-300 font-mono text-[10px] uppercase tracking-wider hover:bg-cyan-950/30 transition cursor-pointer"
                        >
                          Reset Filters
                        </button>
                      </div>
                ...
                {activeTab === "physics" && (
                  <div className="space-y-4 font-sans text-xs">
                    ...
                    <div className="p-4 bg-zinc-950/80 border border-cyan-500/40 space-y-3.5 shadow-lg">
                      <div className="flex items-center justify-between border-b border-zinc-800 pb-2.5">
                        <div className="font-mono font-bold text-cyan-400 text-xs flex items-center gap-2">
                          <Activity className="h-4 w-4 text-cyan-400" />
                          <span>REAL SATELLITE PASS TELEMETRY SPECIMEN (INSAT-3DS / ISS)</span>
                        </div>
                        <span className="px-2.5 py-0.5 text-[9px] font-mono font-bold bg-cyan-950/50 border border-cyan-500/40 text-cyan-300 tracking-wider">
                          CALIBRATED MODEL
                        </span>
                      </div>
                    ...
                    <div className="p-4.5 bg-zinc-950/80 border border-zinc-800 space-y-2.5">
                      <div className="font-mono font-bold text-zinc-200 text-xs uppercase tracking-wider flex items-center gap-2">
                      ...
                      <div className="grid grid-cols-1 sm:grid-cols-3 gap-2.5 font-mono text-[10px] pt-1">
                        <div className="p-3 bg-black border border-zinc-800 text-zinc-300 space-y-1">
                          <span className="text-cyan-400 font-bold block">01. OBSERVER SKY DUSK</span>
                          <span className="text-zinc-400 block text-[10px]">Sun position &le; -6° below local horizon (nautical/astronomical twilight).</span>
                        </div>
                        <div className="p-3 bg-black border border-zinc-800 text-zinc-300 space-y-1">
                          <span className="text-cyan-400 font-bold block">02. DIRECT SUNLIGHT</span>
                          <span className="text-zinc-400 block text-[10px]">Satellite orbit position lies outside Earth's umbral shadow cone.</span>
                        </div>
                        <div className="p-3 bg-black border border-zinc-800 text-zinc-300 space-y-1">
                          <span className="text-cyan-400 font-bold block">03. MAGNITUDE THRESHOLD</span>
                          <span className="text-zinc-400 block text-[10px]">Visual brightness magnitude rating V_mag &le; +6.0.</span>
                        </div>
                      </div>
                    </div>
                  </div>
          ...
          {/* Modal Footer */}
          <div className="px-5 sm:px-6 py-3.5 border-t border-zinc-800 bg-black/90 flex items-center justify-between shrink-0 text-xs font-mono text-zinc-400">
            ...
            <button
              onClick={onClose}
              className="px-5 py-1.5 bg-cyan-500 hover:bg-cyan-400 text-black font-extrabold transition-all shadow-[0_0_20px_rgba(0,229,255,0.3)] hover:shadow-[0_0_25px_rgba(0,229,255,0.5)] active:scale-95 text-xs font-mono tracking-wider cursor-pointer"
            >
              CLOSE MANUAL
            </button>
          </div>
...
function SatelliteTrackerCelestialScene({
  ...
  useFrame(({ camera }, delta) => {
    ...
    // When satellite sight is locked in mission control, direct camera to track target spacecraft
    if (isAimLocked && selectedSat && controlsRef.current) {
      const targetVec = selectedSat.vec3;
      const damp = 1 - Math.exp(-4.5 * Math.min(delta, 0.1));
      controlsRef.current.target.lerp(
        new THREE.Vector3(targetVec.x * 0.35, targetVec.y * 0.35, targetVec.z * 0.35),
        damp
      );
      controlsRef.current.update();
    }
...
export default function StarGazeView({ observer: initialObserver }: StarGazeViewProps) {
  ...
  // Time & Simulation Controls: Default to 1x real-time speed
  const [currentDate, setCurrentDate] = useState<Date>(() => new Date());
  const [isPlaying, setIsPlaying] = useState<boolean>(true);
  const [timeMultiplier, setTimeMultiplier] = useState<number>(1);
  const [simOffsetMinutes, setSimOffsetMinutes] = useState<number>(0);

  // Upper Overlays (Graph & Telemetry Dossier)
  const [showUpperGraph, setShowUpperGraph] = useState<boolean>(false);
  const [showUpperTelemetry, setShowUpperTelemetry] = useState<boolean>(false);
  ...
  // Telescope Track Satellite: Aim sight, direct camera, and open upper overlays
  const handleTrackSatellite = (sat: ComputedSatelliteSkyState) => {
    setSelectedSat(sat);
    setIsAimLocked(true);
    setShowUpperGraph(true);
    setShowUpperTelemetry(true);
    if (isMobileSynced) {
    ...
    if (controlsRef.current) {
      ...
      controlsRef.current.target.set(targetVec.x * 0.35, targetVec.y * 0.35, targetVec.z * 0.35);
      controlsRef.current.update();
    }
  };
  ...
  // 1. Highest elevation satellite overhead (default display target)
  const highestElevationSat = useMemo(() => {
    const overhead = allSatellites.filter((s) => s.isAboveHorizon);
    if (overhead.length > 0) {
      return [...overhead].sort((a, b) => b.elevationDeg - a.elevationDeg)[0];
    }
    const withPass = allSatellites.filter((s) => s.hasUpcomingPassIn24h);
    if (withPass.length > 0) {
      return [...withPass].sort((a, b) => (b.maxPassElevationDeg || 0) - (a.maxPassElevationDeg || 0))[0];
    }
    return allSatellites[0] || null;
  }, [allSatellites]);

  // 2. Line-of-sight detection when phone is synced
  // Golden rectangle: |deltaAz| <= 30° and |deltaEl| <= 10°
  // If multiple satellites lie in same rectangle, pick the one nearest to center (min Math.hypot(deltaAz, deltaEl))
  const phoneGazedSat = useMemo(() => {
    if (!isMobileSynced || !mobileOrientation) return null;
    const phoneHeading = mobileOrientation.heading;
    const phonePitch = mobileOrientation.pitch;

    const candidates = allSatellites
      .map((sat) => {
        const deltaAz = ((sat.azimuthDeg - phoneHeading + 540) % 360) - 180;
        const deltaEl = sat.elevationDeg - phonePitch;
        const distToCenter = Math.hypot(deltaAz, deltaEl);
        const insideRect = Math.abs(deltaAz) <= 30 && Math.abs(deltaEl) <= 10;
        return { sat, deltaAz, deltaEl, distToCenter, insideRect };
      })
      .filter((item) => item.insideRect);

    if (candidates.length === 0) return null;
    candidates.sort((a, b) => a.distToCenter - b.distToCenter);
    return candidates[0].sat;
  }, [isMobileSynced, mobileOrientation, allSatellites]);

  // 3. Active satellite for label, HUD, and overlays
  const activeLabelSat = useMemo(() => {
    if (isAimLocked && detailedSelectedSat) return detailedSelectedSat;
    if (isMobileSynced && mobileOrientation && phoneGazedSat) return phoneGazedSat;
    if (detailedSelectedSat) return detailedSelectedSat;
    return highestElevationSat;
  }, [isAimLocked, detailedSelectedSat, isMobileSynced, mobileOrientation, phoneGazedSat, highestElevationSat]);

  const featuredSat = activeLabelSat;

  return (
    <div className="relative w-full h-full overflow-hidden bg-black text-white font-sans select-none selection:bg-cyan-500/20 selection:text-white">
      ...
      <TopFloatingNav
        ...
        onToggleMobileSightMode={() => setMobileSightMode((prev) => (prev === "ar" ? "track" : "ar"))}
        showLabels={showLabels}
        ...
        onToggleTelemetryPanel={() => setShowUpperTelemetry((prev) => !prev)}
        isTelemetryPanelOpen={showUpperTelemetry || isAimLocked}
        onToggleGraph={() => setShowUpperGraph((prev) => !prev)}
        showGraph={showUpperGraph || isAimLocked}
      />

      {/* ── 2. FULL VIEWPORT 3D CELESTIAL PLANETARIUM DOME CANVAS ── */}
      ...
      {/* Subtle Vignette Gradient Masks for Optimal Editorial Readability */}
      ...
      {(isAimLocked || showUpperGraph) && activeLabelSat && (
        <div className="absolute top-16 left-4 sm:left-8 z-30 w-[350px] sm:w-[430px] max-w-[calc(100vw-2rem)]">
          <MergedPassElevationMagGraph
            ...
            observer={currentObserver}
            currentDate={currentDate}
            onClose={() => setShowUpperGraph(false)}
          />
        </div>
      )}
      ...
      {(isAimLocked || showUpperTelemetry) && activeLabelSat && (
        <div className="absolute top-16 right-4 sm:right-8 z-30 w-[340px] sm:w-[400px] max-w-[calc(100vw-2rem)]">
          <LockedTelemetryCard
            ...
            onClose={() => setShowUpperTelemetry(false)}
          />
        </div>
      )}
        ...
        <div className="absolute bottom-8 sm:bottom-10 left-4 sm:left-8 z-30 pointer-events-auto max-w-sm sm:max-w-md bg-black/90 border border-zinc-800 p-4 shadow-2xl backdrop-blur-md space-y-2.5">
          <div className="inline-flex items-center gap-2 px-2.5 py-0.5 bg-black/90 border border-zinc-700 text-[10px] font-mono uppercase tracking-wider text-zinc-300">
            ...
            <span>
              {isAimLocked
                ...
                : phoneGazedSat
                ? "Phone Sight Target"
                : activeLabelSat.isAboveHorizon
                ? "High-Elevation Spacecraft"
                : "Prime Pass Spacecraft"}
            </span>
          </div>
          ...
          <div>
            ...
            <div className="text-xs font-mono text-cyan-400 tracking-wider">
              {activeLabelSat.category} • NORAD #{activeLabelSat.id}
            </div>
          </div>

          <div className="grid grid-cols-3 gap-2 bg-zinc-950/80 p-2 border border-zinc-900 text-[11px] font-mono">
            <div>
              <span className="text-[9px] text-zinc-500 block uppercase">Elevation</span>
              <span className="text-emerald-400 font-bold">{activeLabelSat.elevationDeg.toFixed(1)}°</span>
            </div>
            <div>
              <span className="text-[9px] text-zinc-500 block uppercase">Azimuth</span>
              <span className="text-white font-bold">{Math.round(activeLabelSat.azimuthDeg)}°</span>
            </div>
            <div>
              <span className="text-[9px] text-zinc-500 block uppercase">Mag</span>
              <span className="text-amber-400 font-bold">
                {activeLabelSat.visualMagnitude ? (activeLabelSat.visualMagnitude > 0 ? `+${activeLabelSat.visualMagnitude.toFixed(1)}` : activeLabelSat.visualMagnitude.toFixed(1)) : "+2.5"} mᵥ
              </span>
            </div>
            <div>
              <span className="text-[9px] text-zinc-500 block uppercase">Altitude</span>
              <span className="text-cyan-300 font-bold">{Math.round(activeLabelSat.satAltitudeKm)} km</span>
            </div>
            <div>
              <span className="text-[9px] text-zinc-500 block uppercase">Status</span>
              <span className="text-zinc-300 font-bold">{activeLabelSat.isSunlit ? "Sunlit" : "Eclipsed"}</span>
            </div>
            <div>
              <span className="text-[9px] text-zinc-500 block uppercase">Orbit</span>
              <span className="text-purple-300 font-bold">{activeLabelSat.inclinationDeg.toFixed(1)}° inc</span>
            </div>
          </div>

          <div className="flex items-center gap-2 pt-1">
            {isAimLocked && detailedSelectedSat?.id === activeLabelSat.id ? (
              <button
                onClick={() => {
                  setIsAimLocked(false);
                  resetDomeCamera();
              ...
              >
                ...
                <span>UNLOCK SIGHT</span>
              </button>
            ) : (
              <button
                onClick={() => handleTrackSatellite(activeLabelSat)}
                className="w-full py-2 bg-white hover:bg-zinc-200 text-black font-bold text-xs font-mono uppercase tracking-wider transition flex items-center justify-center gap-2 cursor-pointer shadow-lg"
              >
                <Target className="h-3.5 w-3.5" />
                <span>AIM / LOCK SIGHT →</span>
              </button>
            )}
          </div>
        </div>
      ...
      {/* ── 6. BOTTOM-RIGHT: 2D PLANISPHERE RADAR (DIRECTLY ABOVE COMPASS) ── */}
      {showRadar && (
        <Planisphere2DRadar
          satellites={allSatellites}
          selectedSat={activeLabelSat}
      ...
      {/* ── 7. BOTTOM-RIGHT: FLOATING COMPASS HUD ── */}
      <FloatingCompassHUD
      ...
      {/* ── 8. BOTTOM-CENTER: SPACECRAFT TIMELINE HUD ── */}
      <BottomSpacecraftHUD
        nakedEyeCount={nakedEyeCount}
        visibleCount={visibleCount}
        activeTrackSatName={isAimLocked && activeLabelSat ? activeLabelSat.name : null}
        currentDate={currentDate}
        isMobileSynced={isMobileSynced}
      ...
      {showManual && (
        <StarGazerManualModal
          initialTab={manualInitialTab}
          onClose={() => setShowManual(false)}
        />
      )}

      {/* Right Telemetry Dock / Catalog Drawer */}
      <RightTelemetryDock
        isOpen={isTelemetryPanelOpen}
        onClose={() => setIsTelemetryPanelOpen(false)}
        satellites={filteredSatellites}
        selectedSat={activeLabelSat}
        onSelectSat={(sat) => setSelectedSat(sat)}
        onAimTrackSat={(sat) => handleTrackSatellite(sat)}
        searchQuery={searchQuery}
        onSearchChange={(q) => setSearchQuery(q)}
        ...
        onRefreshTles={() => {}}
        isRefreshingTles={isRefreshingTles}
        tleStatusText="SGP4 Ephemeris Synced"
      />
    </div>
  );
}
