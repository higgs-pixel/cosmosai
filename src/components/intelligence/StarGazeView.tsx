    ...
    <group position={[0, 0, 0]}>
      ...
      <group ref={domeRef} position={[0, 2.4, 0]}>
        <mesh position={[0, 1.5, 0]}>
          ...
          <meshStandardMaterial color="#0f172a" roughness={0.12} metalness={0.92} emissive="#1e3a5f" emissiveIntensity={0.15} />
        </mesh>
        ...
        <mesh position={[0, 2.2, 3.8]}>
          ...
          <meshStandardMaterial
            ...
            emissive={targetSat ? "#00f0ff" : "#10b981"}
        ...
        <mesh position={[0, 2.2, 2.4]} rotation={[Math.PI / 2, 0, 0]}>
          ...
          <meshStandardMaterial color="#c0c8d8" roughness={0.15} metalness={0.92} emissive="#4a6080" emissiveIntensity={0.4} />
        </mesh>

        {/* 3D Volumetric Sight Cone / Rectangular Projection (HIGH-CONTRAST NEON CYAN) */}
        <mesh position={[0, 2.2, 120]} rotation={[-Math.PI / 2, 0, 0]}>
          <coneGeometry args={[38, 240, 32, 1, true]} />
          <meshBasicMaterial
            color={targetSat ? "#00f0ff" : "#10b981"}
            transparent
            opacity={targetSat ? 0.4 : 0.18}
        ...
        {/* Precision Sight Laser Core Beam (HIGH-CONTRAST NEON CYAN & WHITE CORE) */}
        <Line
      ...
      {/* Direct Line of Sight Beam to Tracked Satellite (NEON CYAN & WHITE CORE) */}
      {targetSat && (
        <>
          <Line
      ...
      <mesh rotation={[-Math.PI / 2, 0, 0]} position={[0, 0.1, 0]}>
        ...
        <meshBasicMaterial color={targetSat ? "#00f0ff" : "#10b981"} transparent opacity={0.25} side={THREE.DoubleSide} />
      </mesh>
        ...
        <div
          className={`px-3.5 py-1 rounded-full text-[10px] font-mono font-black shadow-2xl border backdrop-blur-md whitespace-nowrap animate-pulse transition ${
            mobileOrientation
              ...
              : targetSat
              ? "bg-cyan-950/90 border-cyan-400 text-cyan-300 shadow-[0_0_25px_rgba(0,240,255,0.8)]"
  ...
  return (
    <group>
      ...
      <mesh position={[0, 0, 0]}>
        ...
        <meshBasicMaterial color="#38bdf8" wireframe transparent opacity={0.12} side={THREE.DoubleSide} />
      </mesh>

      <mesh scale={[-1, 1, 1]}>
        <sphereGeometry args={[SKY_RADIUS * 1.05, 32, 32]} />
        <meshBasicMaterial color="#020617" side={THREE.BackSide} transparent opacity={0.75} />
      </mesh>

      {showGrid &&
        altRings.map((pts, idx) => (
          <Line key={idx} points={pts} color="#0284c7" lineWidth={1.3} transparent opacity={0.35} />
        ))}
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-3.5">
                      ...
                      <div className="p-4.5 rounded-xl bg-white/[0.02] border border-white/10 hover:border-emerald-500/30 hover:bg-white/[0.04] transition-all duration-300 space-y-2.5 group">
                        ...
                        <div className="pt-1 flex items-center gap-2 text-[10px] text-slate-400 font-mono">
                          ...
                          <span>Electric Cyan Laser Vector (#00f0ff)</span>
                        </div>
                      </div>
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-3.5 text-slate-300 font-sans">
                        ...
                        <div className="p-3.5 rounded-xl bg-slate-950/50 border border-cyan-500/20 hover:border-cyan-500/40 transition">
                          <div className="font-mono font-bold text-cyan-400 text-xs mb-1 flex items-center gap-2">
                            <Target className="h-3.5 w-3.5 text-cyan-400" />
                            Electric Cyan Laser Vector (#00f0ff)
                          </div>
                          <div className="text-[11px] text-slate-400 leading-relaxed">
...
export default function StarGazeView({ observer: initialObserver }: StarGazeViewProps) {
  ...
  // Telemetry Polling Loop from Mobile to Desktop
  useEffect(() => {
  ...
  const toggle180DomeView = useCallback(() => {
    setIs180DomeView((prev) => {
      ...
      setShowQrModal(true);
      if (next) {
        showToast("🔭 Mode: 180° Free 3D Dome View Active! Scan QR code to sync mobile");
        ...
        showToast("📱 Mobile Compass Sync Active! Scan QR code");
        if (controlsRef.current) {
  ...
  return (
    <div className="flex flex-col md:flex-row w-full h-[calc(100vh-64px)] bg-slate-950 text-white overflow-hidden select-none font-sans">
      ...
      <div className="relative flex-1 h-full min-h-[500px]">
        ...
        <div className="absolute top-4 left-4 right-4 z-[60] flex items-start justify-between gap-2 pointer-events-none">
          {/* Left: Location Picker with Structured Dropdown Menu */}
          <div className="relative pointer-events-auto flex items-center gap-2 bg-slate-950/90 border border-slate-800/90 p-1.5 rounded-2xl backdrop-blur-2xl shadow-2xl shrink-0">
            <button
              onClick={() => setShowLocationMenu((prev) => !prev)}
              className="flex items-center gap-2 bg-slate-900/95 border border-slate-800 hover:border-emerald-500/50 px-3 py-1.5 rounded-xl transition cursor-pointer max-w-[200px] sm:max-w-[280px]"
              title="Click to select observer location"
            >
              <MapPin className="h-4 w-4 text-emerald-400 shrink-0" />
              <span className="text-xs font-semibold text-slate-200 truncate">{currentObserver.name}</span>
              <ChevronDown className={`h-3.5 w-3.5 text-slate-400 shrink-0 transition-transform ${showLocationMenu ? "rotate-180" : ""}`} />
            </button>

            {/* Structured Location Dropdown Menu (Hangs cleanly below, never hides other options) */}
            {showLocationMenu && (
              <div className="absolute top-full mt-2 left-0 w-80 max-h-80 overflow-y-auto bg-slate-950/98 border border-emerald-500/60 rounded-2xl shadow-[0_20px_50px_rgba(0,0,0,0.9),0_0_25px_rgba(16,185,129,0.2)] backdrop-blur-2xl p-2 z-[90] animate-in fade-in slide-in-from-top-2 duration-150">
                <div className="px-2.5 py-1.5 text-[10px] font-mono font-bold text-emerald-400 uppercase tracking-wider border-b border-slate-800 mb-1">
                  Select Ground Observatory Site
                </div>
                <div className="space-y-1">
                  {PRESET_LOCATIONS.map((loc) => {
                    const isSelected = loc.name === currentObserver.name;
                    return (
                      <button
                        key={loc.name}
                        onClick={() => {
                      ...
                      >
                        <div className="flex flex-col min-w-0 pr-2">
                          ...
                          <span className={`text-[10px] font-mono ${isSelected ? "text-emerald-100" : "text-slate-500"}`}>
                            {loc.lat.toFixed(2)}°, {loc.lon.toFixed(2)}° • {loc.altMeters}m
                          </span>
                        </div>
            ...
            <button
              ...
              className="px-3 py-1.5 rounded-xl bg-emerald-950/80 border border-emerald-500/50 text-emerald-300 hover:bg-emerald-600 hover:text-white transition text-xs font-semibold flex items-center gap-1.5 shadow-[0_0_12px_rgba(16,185,129,0.3)] whitespace-nowrap cursor-pointer shrink-0"
          ...
          {/* Right: Layer Controls with Structured Anchored Dropdowns */}
          <div className="pointer-events-auto bg-slate-950/90 border border-slate-800/90 p-1.5 rounded-2xl backdrop-blur-2xl shadow-2xl flex items-center gap-1 overflow-visible shrink-0">
            <button
              onClick={toggle180DomeView}
              className={`px-3 py-1.5 rounded-xl text-xs font-semibold transition flex items-center gap-1.5 whitespace-nowrap shrink-0 ${
                is180DomeView
              ...
              title={
                is180DomeView
                  ? "180° Free 3D View Active"
                  : "Fixed Observer View Active"
              }
            >
            ...
            <div className="relative">
              <button
                ...
                className={`px-3 py-1.5 rounded-xl text-xs font-semibold transition flex items-center gap-1.5 whitespace-nowrap shrink-0 cursor-pointer ${
                  showQrModal
                    ? "bg-emerald-600 text-white font-bold shadow-[0_0_15px_rgba(16,185,129,0.5)]"
                    : "bg-slate-900/90 text-slate-400 hover:text-white"
                }`}
                title="Toggle Mobile Compass QR Sync Code"
              >
              ...
              {/* Anchored QR Sync Dropdown (Drops cleanly below tab, never hides options) */}
              {showQrModal && (
                <div className="absolute top-full mt-2 right-0 w-80 p-4 rounded-2xl bg-slate-950/98 border-2 border-emerald-500/60 shadow-[0_20px_50px_rgba(0,0,0,0.9),0_0_35px_rgba(16,185,129,0.35)] backdrop-blur-2xl font-mono text-xs z-[90] animate-in fade-in slide-in-from-top-2 duration-150 pointer-events-auto">
                  ...
                  <div className="flex flex-col items-center gap-2 my-2">
                    <div className="relative flex flex-col items-center justify-center p-2.5 rounded-xl bg-white border-2 border-emerald-500/60 shadow-[0_0_30px_rgba(16,185,129,0.3)]">
                      <img
                        src="/compass-qr.png"
                    ...
                    <div className="text-[10px] text-slate-300 text-center leading-tight mt-1 font-sans">
            ...
            <button
              ...
              className={`px-3 py-1.5 rounded-xl text-xs font-semibold transition whitespace-nowrap shrink-0 ${
            ...
            {/* Simulate Tab Button with Dropdown Indicator */}
            <div className="relative">
              <button
                ...
                title="Toggle 24-Hour Simulation Dock"
              >
                <Play className="h-3.5 w-3.5 shrink-0" />
                <span>Simulate</span>
                <ChevronDown className={`h-3 w-3 shrink-0 transition-transform ${showSimDock ? "rotate-180" : ""}`} />
              </button>
            </div>
          </div>
        </div>
        ...
        {/* 2D PLANISPHERE RADAR OVERLAY (NEATLY POSITIONED AT BOTTOM-4 LEFT-4) */}
        {showRadar && (
          <div className="absolute bottom-4 left-4 z-30 pointer-events-auto flex flex-col items-center gap-1.5 shadow-2xl">
            <Planisphere2DRadar
              satellites={allSatellites}
              selectedSatId={selectedSat ? selectedSat.id : null}
              onSelectSat={(sat) => handleTrackSatellite(sat)}
        ...
        {showSimDock && (
          <div className={`absolute bottom-4 z-40 w-[340px] sm:w-[420px] max-w-[calc(100vw-32px)] flex flex-col gap-2.5 p-3.5 rounded-2xl bg-slate-950/98 border-2 border-emerald-500/60 backdrop-blur-2xl shadow-[0_15px_45px_rgba(0,0,0,0.85),0_0_35px_rgba(16,185,129,0.35)] pointer-events-auto animate-in fade-in slide-in-from-bottom-2 duration-200 ${
            showRadar ? "left-4 sm:left-[240px]" : "left-4"
          }`}>
            {/* Header label */}
            ...
            <div className="flex items-center justify-between flex-wrap gap-2">
              <div className="flex items-center gap-2">
                ...
                <button
                  ...
                  className="p-1 px-2 rounded-xl bg-slate-900 border border-slate-800 text-slate-300 hover:text-white text-xs font-mono flex items-center gap-1 cursor-pointer"
              ...
              <div className="flex items-center gap-1.5 w-full">
                <button
                  ...
                  className="flex-1 py-1.5 px-2.5 rounded-xl font-bold bg-emerald-600 hover:bg-emerald-500 text-white text-xs transition flex items-center justify-center gap-1 shadow-md cursor-pointer"
                >
                ...
                <button
                  ...
                  className="flex-1 py-1.5 px-2.5 rounded-xl font-bold bg-pink-600 hover:bg-pink-500 text-white text-xs transition flex items-center justify-center gap-1 shadow-[0_0_15px_rgba(236,72,153,0.5)] font-extrabold cursor-pointer"
                >
        ...
        {detailedSelectedSat && (
          <div className="absolute bottom-4 right-4 z-30 w-[calc(100vw-32px)] sm:w-[380px] max-h-[calc(100vh-140px)] overflow-y-auto p-4 rounded-2xl bg-slate-950/98 border-2 border-emerald-500/70 shadow-[0_15px_45px_rgba(0,0,0,0.9),0_0_40px_rgba(16,185,129,0.3)] backdrop-blur-2xl font-sans animate-in fade-in slide-in-from-bottom-2 duration-200 pointer-events-auto">
